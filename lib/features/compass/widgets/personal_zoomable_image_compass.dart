...
/// Personal compass widget with dynamic image selection based on Kua number
class PersonalZoomableImageCompass extends StatefulWidget {
  final double size;
  final double minZoom;
  final double maxZoom;

  const PersonalZoomableImageCompass({
    super.key,
    this.size = 300,
    this.minZoom = 1.0,
    this.maxZoom = 3.0,
  ...
  @override
  State<PersonalZoomableImageCompass> createState() =>
      _PersonalZoomableImageCompassState();
}

class _PersonalZoomableImageCompassState
    extends State<PersonalZoomableImageCompass> with TickerProviderStateMixin {
  ...
  /// Create static compass image based on user's Kua number (DYNAMIC SELECTION)
  Widget _createStaticCompassImage() {
    return Obx(() {
      final controller = Get.find<CompassController>();

      // Get personal compass image path based on Kua number
      String imagePath;
      if (controller.hasPersonalInfo && controller.fengShuiResult != null) {
        final kuaNumber = controller.fengShuiResult!.personalInfo.kuaNumber;
        imagePath = _getCompassImagePath(kuaNumber);
      } else {
        // Fallback to basic compass if no personal info
  ...
  /// Get compass image path based on Kua number (1-9, excluding 5)
  String _getCompassImagePath(int kuaNumber) {
    switch (kuaNumber) {
      case 1:
        return CompassPath.personalCompassKua1;
      case 2:
        return CompassPath.personalCompassKua2;
      case 3:
        return CompassPath.personalCompassKua3;
      case 4:
        return CompassPath.personalCompassKua4;
      case 6:
        return CompassPath.personalCompassKua6;
      case 7:
        return CompassPath.personalCompassKua7;
      case 8:
        return CompassPath.personalCompassKua8;
      case 9:
        return CompassPath.personalCompassKua9;
      default:
        // Fallback to basic compass for invalid Kua numbers
        return CompassPath.baseCompass;
    }
  }
